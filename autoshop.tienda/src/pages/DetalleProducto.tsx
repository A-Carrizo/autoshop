    ...
    useEffect(() => {
        ...
        const cargar = async () => {
            ...
            try {
                ...
                if (!cancelado) toast.error('No se pudo cargar el producto')
            } finally {
        ...
        return (
            <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#f0f3f7' }}>
                ...
                <div style={{ flex: 1, textAlign: 'center', padding: '80px 20px', color: COL.muted }}>
                    ...
                    <p style={{ fontSize: '15px', marginBottom: '20px' }}>Este producto no esta disponible.</p>
        ...
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#f0f3f7' }}>
            ...
            <main style={{ flex: 1, maxWidth: '1440px', margin: '0 auto', padding: '28px 32px', width: '100%' }}>
                ...
                <div style={{
                    ...
                    {/* Imagen */}
                    <div style={{
                        position: 'relative', background: '#f7fafc', borderRadius: '12px',
                        minHeight: '380px', display: 'flex', alignItems: 'center', justifyContent: 'center',
                        overflow: 'hidden',
                    }}>
                        {producto.imagenUrl ? (
                            <img src={`${API.imagenesBase}${producto.imagenUrl}`} alt={producto.nombre}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <span style={{ color: COL.primary, fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '8px' }}>
                            {producto.categoriaNombre}
                        </span>
                        <h1 style={{ fontSize: '24px', fontWeight: 700, color: '#1a202c', margin: '0 0 16px', lineHeight: 1.3 }}>
                            {producto.nombre}
                        </h1>

                        <div style={{ marginBottom: '20px' }}>
                            {producto.descuentoPct > 0 && (
                                <div style={{ color: '#a0aec0', fontSize: '15px', textDecoration: 'line-through' }}>
                                    Gs. {fmt(producto.precioOriginal)}
                                </div>
                            )}
                            <div style={{ color: COL.primary, fontSize: '32px', fontWeight: 800 }}>
                                Gs. {fmt(producto.precioVenta)}
                            </div>
                        </div>

                        {producto.descripcion && (
                            <p style={{ color: '#718096', fontSize: '14px', lineHeight: 1.7, marginBottom: '24px' }}>
                                {producto.descripcion}
                            </p>
                        )}
                        ...
                        <div style={{ marginBottom: '8px' }}>
                            {producto.enStock ? (
                                <span style={{ color: '#2f855a', fontSize: '13px', fontWeight: 600 }}>
                        ...
                        {producto.enStock && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                                <span style={{ fontSize: '13px', color: '#2d3748', fontWeight: 600 }}>Cantidad:</span>
                                <div style={{ display: 'flex', alignItems: 'center', border: `1.5px solid ${COL.border}`, borderRadius: '8px' }}>
                                    <button onClick={() => setCantidad(c => Math.max(1, c - 1))}
                                        style={{ width: '32px', height: '32px', border: 'none', background: 'none', cursor: 'pointer', fontSize: '14px', color: '#2d3748' }}>
                                        <i className="fas fa-minus"></i>
                                    </button>
                                    <span style={{ width: '40px', textAlign: 'center', fontSize: '14px', fontWeight: 600 }}>{cantidad}</span>
                                    <button onClick={() => setCantidad(c => Math.min(producto.stockActual, c + 1))}
                                        style={{ width: '32px', height: '32px', border: 'none', background: 'none', cursor: 'pointer', fontSize: '14px', color: '#2d3748' }}>
                        ...
                        <button
                            ...
                            style={{
                                ...
                                background: producto.enStock ? COL.primary : '#cbd5e0',
