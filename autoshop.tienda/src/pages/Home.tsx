...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const cargar = async () => {
            ...
            for (let intento = 1; intento <= MAX_INTENTOS; intento++) {
                const exito = await intentarCargar()
                if (cancelado) return
                if (exito) break
                if (intento === MAX_INTENTOS) {
                    toast.error('No se pudieron cargar los productos.')
                } else { await esperar(ESPERA_MS) }
            }
    ...
    const handleAgregar = (p: Producto) => {
        ...
        toast.success(`${p.nombre} ${t.agregar}`)
    }
            ...
            <main style={{ flex: 1, maxWidth: '1440px', margin: '0 auto', padding: '28px 32px', width: '100%' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
                    <h1 style={{ fontSize: '20px', fontWeight: 700, color: '#1a202c', margin: 0 }}>
                        {busqueda ? `${t.resultadosPara} "${busqueda}"` : t.todosLosProductos}
                    </h1>
                    <select value={orden} onChange={e => setOrden(e.target.value)}
                        style={{ padding: '8px 12px', borderRadius: '8px', border: `1.5px solid ${COL.border}`, fontSize: '13px', color: '#2d3748', outline: 'none', background: '#fff' }}>
                ...
                {cargando ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '20px' }}>
                        {productos.map(p => (
                            <div key={p.id}
                                style={{ background: '#fff', borderRadius: '12px', overflow: 'hidden', border: `1px solid ${COL.border}`, display: 'flex', flexDirection: 'column', transition: 'transform 0.2s ease, box-shadow 0.2s ease', cursor: 'default' }}
                                onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-6px)'; e.currentTarget.style.boxShadow = '0 12px 24px rgba(204,0,0,0.15)' }}
                            ...
                            >
                                <Link to={`/producto/${p.id}`} style={{ textDecoration: 'none' }}>
                                    <div className="tienda-img-zoom-container" style={{ position: 'relative', height: '160px', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: '1px solid #f0f0f0', overflow: 'hidden' }}>
                                        {p.imagenUrl ? (
                                            <img src={`${API.imagenesBase}${p.imagenUrl}`} alt={p.nombre} className="tienda-img-zoom" style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.3s ease' }} />
                                        ) : (
                                            <i className="fas fa-image" style={{ fontSize: '36px', color: '#cbd5e0' }}></i>
                                        )}
                                        {p.descuentoPct > 0 && (
                                            <span style={{ position: 'absolute', top: '8px', left: '8px', background: COL.accent, color: '#ffffff', fontSize: '11px', fontWeight: 700, padding: '3px 8px', borderRadius: '6px' }}>
                                                -{p.descuentoPct}%
                                            </span>
                                        )}
                                        {!p.enStock && (
