    ...
    useEffect(() => {
        ...
        const cargar = async () => {
            try {
                ...
                if (!cancelado) toast.error('No se pudieron cargar tus pedidos')
            } finally {
            ...
            <main style={{ flex: 1, maxWidth: '900px', margin: '0 auto', padding: '28px 32px', width: '100%' }}>
                ...
                {cargando ? (
                    ...
                    <div style={{
                        ...
                        <p style={{ fontSize: '15px', marginBottom: '20px' }}>Todavia no realizaste ningun pedido.</p>
                        <Link to="/" style={{
                    ...
                    <div style={{ background: '#fff', borderRadius: '16px', border: `1px solid ${COL.border}`, overflow: 'hidden' }}>
                        {pedidos.map((p, idx) => {
                            ...
                            return (
                                <Link key={p.id} to={`/mis-pedidos/${p.id}`} style={{
                                    display: 'flex', alignItems: 'center', gap: '16px', padding: '18px 20px',
                                    borderBottom: idx < pedidos.length - 1 ? `1px solid ${COL.border}` : 'none',
                                    textDecoration: 'none', color: 'inherit',
                                }}>
                                    <div style={{ flex: 1 }}>
                                        <p style={{ fontSize: '14px', fontWeight: 700, color: '#1a202c', margin: '0 0 4px' }}>{p.numeroPedido}</p>
                                        <p style={{ fontSize: '12px', color: COL.muted, margin: 0 }}>
                                            {fmtFecha(p.fecha)} · {p.cantidadItems} producto{p.cantidadItems !== 1 ? 's' : ''}
                                        </p>
                                    </div>
                                    <span style={{
                                    ...
                                    <p style={{ fontSize: '14px', fontWeight: 700, color: '#1a202c', width: '110px', textAlign: 'right', margin: 0 }}>
                                        Gs. {fmt(p.total)}
                                    </p>
